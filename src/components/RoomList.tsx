...
const PLACEHOLDERS = [
  "Project Moonshot", "The Refactor Quest", "Sprint 42 Planning",
  "Operation Ship It", "The Great Backlog Grooming",
];

export function RoomList({ userName, onJoinRoom }: { userName: string; onJoinRoom: (id: string) => void }) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setRoomName("");
      onJoinRoom(room.id);
    } catch { toast.error("Couldn't create room"); }
  ...
  const handleDelete = async (room: Room) => {
    ...
    setDeleting(room.id);
    try { await api().deleteRoom(room.id); toast.success("Table closed"); }
    catch { toast.error("Couldn't delete room"); }
  ...
  return (
    <div className="space-y-6">
      <div className="text-center pt-2">
        <h1 className="text-4xl sm:text-5xl font-display font-bold text-slate-800 mb-2">Shall we plan?</h1>
        <p className="text-slate-600 text-lg">Spin up a new table or slide into an existing one</p>
      </div>
      ...
      <div className="bg-white/90 backdrop-blur rounded-3xl shadow-xl p-6 border-2 border-white">
        ...
        {rooms === undefined ? (
          <div className="flex justify-center py-10"><span className="text-3xl animate-bounce">🎲</span></div>
        ) : rooms.length === 0 ? (
                ...
                <div className="flex items-center gap-2">
                  {room.creatorName === userName && (
                    <button
                      ...
                      className="w-8 h-8 flex items-center justify-center text-slate-300 hover:text-rose-400 transition-colors disabled:opacity-40"
                      title="Delete room"
                    >
                      {deleting === room.id ? "…" : "✕"}
                    </button>
